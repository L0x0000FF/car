 ...
 * @return {*}
 */
SFIIC_StatusTypeDef writeReadBitSet(SFIIC_HandleTypeDef *hsfi2c,
                                    uint8_t address, int selection,
                                    int32_t timeOut) {
  GPIO_TypeDef *SDA = hsfi2c->SDAPort;
  uint32_t SDAP = hsfi2c->SDAPin;
  GPIO_TypeDef *SCL = hsfi2c->SCLPort;
  ...
  if (waitACK(hsfi2c, timeOut) == SFIIC_OK) successfullACK = 1;
  if (selection == 1) changeSDAState(hsfi2c, 1);
 ...
 * @return {*}
 */
SFIIC_StatusTypeDef writeBytes(SFIIC_HandleTypeDef *hsfi2c, uint8_t value,
                               int32_t timeOut) {
  GPIO_TypeDef *SDA = hsfi2c->SDAPort;
  uint32_t SDAP = hsfi2c->SDAPin;
  GPIO_TypeDef *SCL = hsfi2c->SCLPort;
 ...
 * @return {*}
 */
SFIIC_StatusTypeDef SFIIC_Mem_Write(SFIIC_HandleTypeDef *hsfi2c,
                                    uint16_t DevAddress, uint16_t MemAddress,
                                    uint16_t MemAddSize, uint8_t *pData,
                                    uint16_t Size, uint32_t Timeout) {
  if (!(writeReadBitSet(hsfi2c, DevAddress, 1, Timeout) ==
        SFIIC_OK))  //写入器件地址
  {
    resetIO(hsfi2c);
    return SFIIC_BUSY;
  ...
  {
    if (!(writeBytes(hsfi2c, MemAddress, Timeout) ==
          SFIIC_OK))  //写入寄存器地址
    {
      resetIO(hsfi2c);
      return SFIIC_ERROR;
 ...
 * @return {*}
 */
SFIIC_StatusTypeDef SFIIC_Mem_Read(SFIIC_HandleTypeDef *hsfi2c,
                                   uint16_t DevAddress, uint16_t MemAddress,
                                   uint16_t MemAddSize, uint8_t *pData,
                                   uint16_t Size, uint32_t Timeout) {
 ...
 * @return {*}
 */
SFIIC_StatusTypeDef SFIIC_Master_Transmit(SFIIC_HandleTypeDef *hsfi2c,
 ...
 * @return {*}
 */
SFIIC_StatusTypeDef SFIIC_Master_Receive(SFIIC_HandleTypeDef *hsfi2c,
                                         uint16_t DevAddress, uint8_t *pData,
                                         uint16_t Size, uint32_t Timeout) {
  if (!(writeReadBitSet(hsfi2c, DevAddress, 0, Timeout) == SFIIC_OK)) {
    resetIO(hsfi2c);

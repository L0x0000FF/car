...
typedef struct BSP_CanTypeDef_t {
  ...
  uint32_t tx_mailbox;
  std::vector<can_rx_callback> call_backs;
...
BSP_CanTypeDef
    can_devices[DEVICE_CAN_CNT];  //定义对应数量的can外设，即代码中定义can总线
...
void BSP_CAN_Init() {
  ...
  // can_devices[0].call_backs = cvector_create(sizeof(
  //     can_rx_callback));  //创建用于储存回调函数的向量组，存储方式为其头指针
  ...
  for (int i = 0; i < DEVICE_CAN_CNT; ++i) {
    HAL_CAN_Start(
        can_devices[i].device);  //将HAL库的对应功能函数封装，实现映射和隔离
    HAL_CAN_ActivateNotification(can_devices[i].device,
...
void BSP_CAN_Send(uint8_t can_id, uint16_t identifier, uint8_t *data,
  ...
  txconf.DLC = len;
  uint32_t can_tx_start_time = usTimer.getTime() / 1000;
  while (HAL_CAN_GetTxMailboxesFreeLevel(can_devices[can_id].device) == 0) {
    // HAL_CAN_AbortTxRequest
    uint32_t can_tx_now_time = usTimer.getTime() / 1000;
    // 2ms还没有发送出去，放弃发送
    if (can_tx_now_time > can_tx_start_time + 2) {
      HAL_CAN_AbortTxRequest(can_devices[can_id].device, CAN_TX_MAILBOX0 |
                                                             CAN_TX_MAILBOX1 |
                                                             CAN_TX_MAILBOX2);
      __NOP();
      break;
...
void update_filter(uint8_t can_id, uint32_t filter_index) {
  ...
  tmp.FilterFIFOAssignment =
      (group_index & 1) ? CAN_RX_FIFO0
                        : CAN_RX_FIFO1;  // 4个一组，按照组别设置FIFO，平衡负载
...
void BSP_CAN_RegisterRxCallback(uint8_t can_id, can_rx_callback func) {
  can_devices[can_id].call_backs.push_back(func);
...
void BSP_CAN_FifoMsg_Callback(CAN_HandleTypeDef *hcan, uint32_t fifo) {
  for (int i = 0; i < DEVICE_CAN_CNT; ++i) {
    if (hcan == can_devices[i].device) {
      ...
      for (size_t j = 0; j < can_devices[i].call_backs.size(); ++j) {
        can_rx_callback funcnow = can_devices[i].call_backs[j];
            //*(can_rx_callback *)cvector_val_at(can_devices[i].call_backs, j);
